...
- The preview system expects a web server on port 3030 to be ready and respond with HTTP 200 OK at a healthcheck path.
- This project runs Expo dev server using `npm run web`. For CI/preview:
  - The script order is:
    ...
    2) `preview-healthcheck-launcher.cjs` — immediately starts a background Express server that binds to `0.0.0.0` and responds 200 at:
       - Port: `EXPO_PUBLIC_PORT` (default `3030`)
       - Path: `EXPO_PUBLIC_HEALTHCHECK_PATH` (default `/healthz`)
    3) `expo start --web --port ${EXPO_PUBLIC_PORT:-3030}` — starts the Expo web dev server.
